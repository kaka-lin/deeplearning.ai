...
def predict(sess, image_path, image_file):
    ...
    image, image_data = preprocess_image(image_path + image_file, model_image_size = (608, 608))
...
def video_detection(sess, image):
    resized_image = cv2.resize(image, (608, 608), interpolation=cv2.INTER_AREA)
...
if __name__ == "__main__":
    ...
    out_scores, out_boxes, out_classes = predict(sess, image_path, image_file)  
    ...
    camera.set(cv2.CAP_PROP_FRAME_WIDTH, 320) # 設計解析度
